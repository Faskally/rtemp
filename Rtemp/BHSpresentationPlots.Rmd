...
## BHS presentation figures and text

```{r setup, cache = FALSE, message = FALSE, echo = FALSE, purl = FALSE}
opts_chunk $ set(fig.width = 10, fig.height = 10, 
                 fig.path   = "B:/Loch_Ard_Felling/BHS/fig/",
                 cache = TRUE,
                 cache.path = "B:/Loch_Ard_Felling/BHS/cache/",
                 echo = FALSE,
                 warning = FALSE,
                 message = FALSE
                 ) 
...
simfits <- sapply(1:n, function(i) X %*% rbind(sim1[,i], sim2[,i], sim3[,i], sim4[,i]))
dim(simfits) <- c(24, 2138, n)
...
```{r save, purl = FALSE, cache = FALSE}
save(list = ls(), file = "B:/Loch_Ard_Felling/Package/rivertemp/R/currentImage.Rdata") 
```
